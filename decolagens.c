...
char verificarDestino(const char destino[]) {
    FILE *arquivo = fopen("destino.txt", "r");
    if (arquivo == NULL) {
        printf("Erro ao abrir o arquivo de destinos!\n");
        return '0'; // Retorna '0' para erro de leitura
    }
    ...
    while (fgets(linha, sizeof(linha), arquivo)) {
        ...
        // Compara o destino fornecido com a linha do arquivo
        if (strcmp(linha, destino) == 0) {
            fclose(arquivo);
    ...
    fclose(arquivo);
    return '2'; // Destino não encontrado
}
